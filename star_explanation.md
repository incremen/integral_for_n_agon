...
Again, the constant at the beginning is to make the star fit in a constant size, ignore it for now:
...
Since the integral is linear, we pull $L$ out front as a constant scalar — it rescales the whole path to lock the outer circumradius at 1.
